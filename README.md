...
- `repl.py` — interactive REPL exposing `submit`, `bcast`, `pmap`.
- `run.sh` — spawns SSH workers (one per line of `hosts.txt`) then runs the script.
- `hosts.txt` — one host per line.

## Setup

On every host in `hosts.txt`, install matching Python + cloudpickle in `~/miniforge3`:
...
'hal got 42'
>>> bcast(f, 4, 99)
['hal got 99', 'glados got 99', 'hal got 99', 'glados got 99']
>>> pmap(f, range(4))
...
2. For each host in `hosts.txt`, `ssh -R /tmp/m-N.sock:/tmp/m.sock` reverse-tunnels a fresh socket on the remote back to the driver's socket.
